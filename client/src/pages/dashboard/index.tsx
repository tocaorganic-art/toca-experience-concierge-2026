          ...
          {bookings.map((b: any) => (
            <TableRow key={b.id}>
              ...
              <TableCell className="hidden sm:table-cell">
                {b.check_in ? new Date(b.check_in).toLocaleDateString("pt-BR") : "—"}
              </TableCell>
              <TableCell className="hidden sm:table-cell">
                {b.check_out ? new Date(b.check_out).toLocaleDateString("pt-BR") : "—"}
              </TableCell>
        ...
        <div key={r.id} className="flex items-start gap-3 p-3 rounded-lg bg-muted/40">
          <div className="flex gap-0.5 shrink-0 mt-0.5">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className={`h-3 w-3 ${i < r.estrelas ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/30"}`}
              />
            ))}
          </div>
...
function StatsCards() {
  ...
  const cards = [
    ...
    },
    {
      ...
      value: isLoading ? "..." : (counters?.clientes_satisfeitos ?? 0),
      icon: Users,
      description: "acumulado",
    },
    {
      title: "Nota Média",
      value: reviewStats ? `${Number(reviewStats.average).toFixed(1)} ★` : "—",
      icon: Star,
      description: `${reviewStats?.total ?? 0} avaliações`,
    },
  ];
      ...
      <div className="space-y-6 max-w-6xl mx-auto">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Toca Experience Concierge — painel de controle
          </p>
        </div>
